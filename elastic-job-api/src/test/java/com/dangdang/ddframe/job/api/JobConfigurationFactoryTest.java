...
public final class JobConfigurationFactoryTest {
    ...
    public void assertCreateSimpleJobConfiguration() {
        assertThat(JobConfigurationFactory.createSimpleJobConfigurationBuilder("simpleJob", FooSimpleElasticJob.class, "0/10 * * * *", 10).build(), is(instanceOf(SimpleJobConfiguration.class)));
    }

    @Test
    public void assertCreateDataflowJobConfiguration() {
        assertThat(JobConfigurationFactory.createDataflowJobConfigurationBuilder("dataflowJob", TestDataflowJob.class, "0/10 * * * *", 10, DataflowJobConfiguration.DataflowType.THROUGHPUT).build(), 
                is(instanceOf(DataflowJobConfiguration.class)));
    }

    @Test
    public void assertCreateScriptJobConfiguration() {
        assertThat(JobConfigurationFactory.createScriptJobConfigurationBuilder("scriptJob", "0/10 * * * *", 10, "test.sh").build(), is(instanceOf(ScriptJobConfiguration.class)));
    }
}
