...
public final class TaskContextTest {
    ...
    public void assertNew() {
        ...
        Assert.assertThat(actual.getMetaInfo().getJobName(), is("test_job"));
        Assert.assertThat(actual.getMetaInfo().getShardingItems().get(0), is(0));
        Assert.assertThat(actual.getType(), is(ExecutionType.READY));
        Assert.assertThat(actual.getSlaveId(), is("slave-S0"));
    ...
    public void assertNewWithoutSlaveId() {
        ...
        Assert.assertThat(actual.getSlaveId(), is("unassigned-slave"));
    }
    
    @Test
    public void assertGetMetaInfo() {
        TaskContext actual = new TaskContext("test_job", Lists.newArrayList(0), ExecutionType.READY, "slave-S0");
        Assert.assertThat(actual.getMetaInfo().toString(), is("test_job@-@0"));
    }
    
    @Test
    public void assertTaskContextFrom() {
        TaskContext actual = TaskContext.from(TaskNode.builder().build().getTaskNodeValue());
        Assert.assertThat(actual.getId(), is(TaskNode.builder().build().getTaskNodeValue()));
        Assert.assertThat(actual.getMetaInfo().getJobName(), is("test_job"));
        Assert.assertThat(actual.getMetaInfo().getShardingItems().get(0), is(0));
        Assert.assertThat(actual.getType(), is(ExecutionType.READY));
        Assert.assertThat(actual.getSlaveId(), is("slave-S0"));
    ...
    public void assertMetaInfoFromWithMetaInfo() {
        ...
        Assert.assertThat(actual.getShardingItems().get(0), is(1));
    }
    ...
    public void assertGetIdForUnassignedSlave() {
        Assert.assertThat(TaskContext.getIdForUnassignedSlave("test_job@-@0@-@READY@-@slave-S0@-@0"), is("test_job@-@0@-@READY@-@unassigned-slave@-@0"));
    }
    
    @Test
    public void assertGetTaskName() {
        TaskContext actual = TaskContext.from(TaskNode.builder().build().getTaskNodeValue());
        Assert.assertThat(actual.getTaskName(), is("test_job@-@0@-@READY@-@slave-S0"));
    }
    
    @Test
    public void assertGetExecutorId() {
        TaskContext actual = TaskContext.from(TaskNode.builder().build().getTaskNodeValue());
        Assert.assertThat(actual.getExecutorId("app"), is("app@-@slave-S0"));
    }
    ...
    public void assertSetSlaveId() {
        ...
        Assert.assertThat(actual.getSlaveId(), is("slave-S1"));
    }
