...
public final class JobEventRdbSearch {
    ...
    private static final List<String> FIELDS_JOB_EXECUTION_LOG = 
            Arrays.asList("id", "hostname", "ip", "execution_id", "task_id", "job_name", "execution_source", "sharding_item", "start_time", "complete_time", "is_success", "failure_cause");
    
    private static final List<String> FIELDS_JOB_STATUS_TRACE_LOG = 
            Arrays.asList("id", "job_name", "execution_id", "original_task_id", "task_id", "slave_id", "source", "execution_type", "sharding_item", "state", "message", "creation_time");
    ...
    private List<JobExecutionEvent> getJobExecutionEvents(final Condition condition) {
        ...
        try (
            ...
            while (resultSet.next()) {
                JobExecutionEvent jobExecutionEvent = new JobExecutionEvent(resultSet.getString(1), resultSet.getString(2), resultSet.getString(3), resultSet.getString(4), resultSet.getString(5),
                        resultSet.getString(6), JobExecutionEvent.ExecutionSource.valueOf(resultSet.getString(7)), Integer.parseInt(resultSet.getString(8)),
                        new Date(resultSet.getTimestamp(9).getTime()), resultSet.getTimestamp(10) == null ? null : new Date(resultSet.getTimestamp(10).getTime()),
        ...
        try (
            ...
            while (resultSet.next()) {
                JobStatusTraceEvent jobStatusTraceEvent = new JobStatusTraceEvent(resultSet.getString(1), resultSet.getString(2), resultSet.getString(3), resultSet.getString(4),
                        resultSet.getString(5), resultSet.getString(6), JobStatusTraceEvent.Source.valueOf(resultSet.getString(7)), resultSet.getString(8), resultSet.getString(9),
                        JobStatusTraceEvent.State.valueOf(resultSet.getString(10)), resultSet.getString(11), new Date(resultSet.getTimestamp(12).getTime()));
