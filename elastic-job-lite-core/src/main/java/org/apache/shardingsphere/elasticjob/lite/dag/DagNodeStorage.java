...
public class DagNodeStorage {
     ...
     * @param groupName dag group name
     * @param jobName dag job name
     */
    public DagNodeStorage(final CoordinatorRegistryCenter regCenter, final String groupName, final String jobName) {
    ...
    /**
     * Persist /dag/groupName/config/jobName ,value=job's dependencies with comm split.
     *
    ...
    private String pathOfDagGraph() {
        return String.format(DAG_GRAPH, groupName);
    }

    private String pathOfDagGraphJob(final String jobName) {
        return String.format(DAG_GRAPH_JOB, groupName, jobName);
    }

    private String pathOfDagGraphJobRetryTimes() {
        return String.format(DAG_GRAPH_JOB_RETRYTIMES, groupName, jobName);
    }

    private String pathOfDagRunning() {
        return String.format(DAG_RUNNING, groupName);
    }

    private String pathOfDagRunningJob(final String jobName) {
        return String.format(DAG_RUNNING_JOB, groupName, jobName);
    }

    private String pathOfDagSuccess() {
        return String.format(DAG_SUCCESS, groupName);
    }

    private String pathOfDagSuccessJob(final String jobName) {
        return String.format(DAG_SUCCESS_JOB, groupName, jobName);
    }

    private String pathOfDagFail() {
        return String.format(DAG_FAIL, groupName);
    }

    private String pathOfDagFailJob(final String jobName) {
        return String.format(DAG_FAIL_JOB, groupName, jobName);
    }

    private String pathOfDagSkip() {
        return String.format(DAG_SKIP, groupName);
    }

    private String pathOfDagSkipJob(final String jobName) {
        return String.format(DAG_SKIP_JOB, groupName, jobName);
    }

    private String pathOfDagRetry() {
        return String.format(DAG_RETRY, groupName);
    }

    private String pathOfDagRetryJob(final String jobName) {
        return String.format(DAG_RETRY_JOB, groupName, jobName);
    }
    ...
    public String pathOfJobNodeState() {
        return String.format("/%s/%s", jobName, JobStateNode.ROOT_STATE_FOR_CACHE);
    }
    ...
    public void initDagGraph(final Map<String, Set<String>> allDagNode, final String batchNo) {
        log.debug("Dag-{}, before create Dag Graph, clean exist path.", groupName);
        // clean
        ...
        log.debug("Dag-{}, Create Dag Graph, create path.", groupName);
        // create path
        ...
        log.debug("Dag-{}, Create Dag Graph, create graph.", groupName);
        // init graph
        ...
        log.info("Dag-{}, Create Dag Graph success.", groupName);
    }

    /**
     * Get value of /dag/groupName/graph.
     *
     * @return batch no.
     */
    ...
    /**
     * get value of /dag/groupName/states.
     *
     * @return dag state
     */
    ...
    /**
     * update value of /dag/groupName/states.
     *
    ...
    /**
     * running:/dag/groupname/running/{}.
     * success:/dag/groupname/success/{}.
     * fail:/dag/groupname/fail/{}.
     *
    ...
    /**
     * Get /dag/groupName/config/all config jobs with their dependencies.
     *
    ...
    /**
     * Get /dag/groupName/graph/all nodes with their dependencies.
     *
    ...
    public void triggerJob(final String job) {
        if (isJobTriggered(job)) {
            log.debug("Dag-{} Job-{} has already been tiggered!", groupName, job);
            return;
        }
        log.debug("Dag-{} trigger job-[{}] in transaction.", groupName, job);
        try {
            ...
            //CHECKSTYLE:ON
            log.debug("Dag-{}[{}] trigger job in transaction Exception！", groupName, job, exp);
        }

        log.info("Dag-{}[{}] has been triggered [{}]", groupName, job, isJobTriggered(job));
        printZkPath();
    }
    ...
    public void triggerRetryJob() {
        log.debug("Dag-{} trigger RETRY job-[{}] in transaction.", groupName, jobName);
        try {
            ...
            //CHECKSTYLE:ON
            log.debug("Dag-{}[{}] trigger RETRY job in transaction Exception！", groupName, jobName, exp);
        }

        log.info("Dag-{}[{}] RETRY job has been triggered!", groupName, jobName);
        printZkPath();
    }

    private void printZkPath() {
        if (log.isDebugEnabled()) {
            log.debug("Dag-{}[{}] after trigger:", groupName, jobName);
            log.debug("Dag-{}[{}] ZK path of SUCC  : {}", groupName, jobName, getDagJobListByState(DagJobStates.SUCCESS));
            log.debug("Dag-{}[{}] ZK path of RUN   : {}", groupName, jobName, getDagJobListByState(DagJobStates.RUNNING));
            log.debug("Dag-{}[{}] ZK path of FAIL  : {}", groupName, jobName, getDagJobListByState(DagJobStates.FAIL));
            log.debug("Dag-{}[{}] ZK path of SKIP  : {}", groupName, jobName, getDagJobListByState(DagJobStates.SKIP));
            log.debug("Dag-{}[{}] ZK path of RETRY : {}", groupName, jobName, getDagJobListByState(DagJobStates.RETRY));
        }
    }
     ...
     * Persist jobName to path '/dag/groupName/retry'
     *
     * @param i retry times.
     */
     ...
     * Get dag root groups.
     *
     * @return list fo dag group name
     */
    public List<String> getAllDagGroups() {
