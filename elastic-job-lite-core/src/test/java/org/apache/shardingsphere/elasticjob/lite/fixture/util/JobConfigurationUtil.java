...
public final class JobConfigurationUtil {
    ...
    public static JobConfiguration createSimpleJobConfiguration() {
        return JobConfiguration.newBuilder(JobCoreConfiguration.newBuilder("test_job", JobType.SIMPLE, "0/1 * * * * ?", 3).build()).build();
    }
    ...
    public static JobConfiguration createSimpleJobConfiguration(final boolean overwrite) {
        return JobConfiguration.newBuilder(JobCoreConfiguration.newBuilder("test_job", JobType.SIMPLE, "0/1 * * * * ?", 3).build()).overwrite(overwrite).build();
    }
}
