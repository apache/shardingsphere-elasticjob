...
public final class ShardingServiceTest {
    ...
    public void assertShardingNecessaryWhenMonitorExecutionEnabledAndIncreaseShardingTotalCount() {
        ...
        when(configService.load(false)).thenReturn(
                JobConfiguration.newBuilder(new JobTypeConfiguration(JobCoreConfiguration.newBuilder("test_job", JobType.SIMPLE, "0/1 * * * * ?", 3).build())).monitorExecution(true).build());
    ...
    public void assertShardingNecessaryWhenMonitorExecutionDisabledAndDecreaseShardingTotalCount() {
        ...
        when(leaderService.isLeaderUntilBlock()).thenReturn(true);
        when(configService.load(false)).thenReturn(
                JobConfiguration.newBuilder(new JobTypeConfiguration(JobCoreConfiguration.newBuilder("test_job", JobType.SIMPLE, "0/1 * * * * ?", 3).build())).monitorExecution(false).build());
    ...
    public void assertGetShardingItemsWithEnabledServer() {
        ...
        when(configService.load(true)).thenReturn(
                JobConfiguration.newBuilder(new JobTypeConfiguration(JobCoreConfiguration.newBuilder("test_job", JobType.SIMPLE, "0/1 * * * * ?", 3).build())).build());
